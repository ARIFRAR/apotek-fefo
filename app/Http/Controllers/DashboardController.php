...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        // Pemberitahuan obat akan expire dalam 30 hari
        $goToExpire = DetailPembelian::where('tanggal_expire', '>', date('Y-m-d'))
                ->where('tanggal_expire', '<=', date('Y-m-d', strtotime('+30 days')))
                ->where('jumlah_sisa', '>', 0)
                ->get();

        // Hitung selisih tanggal expire dengan tanggal hari ini
        $sisa = [];
        foreach ($goToExpire as $key => $value) {
